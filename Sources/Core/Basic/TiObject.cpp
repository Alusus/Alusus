...
{
  static Core::Basic::ObjectTypeInfo *type_info = 0;
  if (type_info == 0) {
    ...
    type_info = reinterpret_cast<Core::Basic::ObjectTypeInfo*>(GLOBAL_STORAGE->getObject(uniqueName));
    if (type_info == 0) {
      type_info = new Core::Basic::ObjectTypeInfo(my_type, typeNamespace, moduleName, url, 0, 0);

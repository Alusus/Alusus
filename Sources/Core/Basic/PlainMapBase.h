...
template<class CTYPE, class PTYPE> class PlainMapBase : public PTYPE, public virtual DynamicMapContaining<CTYPE>
{
  ...
  TEMPLATE_TYPE_INFO(PlainMapBase, PTYPE, "Core.Basic", "Core", "alusus.net", (CTYPE, PTYPE), (
   ...
   * binary search.
   */
  public: PlainMapBase(Bool useIndex = false) : inherited(0), base(0)
  {
  ...
  public: PlainMapBase(const std::initializer_list<Argument> &args, Bool useIndex = false)
    : inherited(0), base(0)
  {
    if (useIndex) this->index = new Index(&this->list);
    else this->index = 0;

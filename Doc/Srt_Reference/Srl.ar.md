...
#### مثل_سندنا (castSrdRef)

ماكرو يُستخدم لتمثيل سند من صنف `سـندنا` كسند بصنف آخر، بشكل مشابه لـ `مثل_سند` لكن للسندات المشتركة. يستلم معطيين، الأول
هو متغير `سـندنا` المراد تمثيله، والثاني هو صنف الكائن الهدف. يشارك السند الناتج نفس عداد السندات مع السند الأصلي، لذا فإن
تحرير أي منهما يعمل بشكل صحيح بغض النظر عن أيهما يُحرر أولا، ولا يُحرر الكائن الأصلي إلا بعد تحرير آخر سند من السندين.

```
عرف س: سـندنا[صـنفي] = سـندنا[صـنفي].أنشئ()؛
عرف ص: سـندنا[صـنف_فرعي] = مثل_سندنا[س، صـنف_فرعي]؛
```
...
ماكرو يحجز ذاكرة لكائن من الصنف المعطى دون تهيئته، ويعيد سندًا لذلك الكائن. يستلم معطىً واحدًا هو صنف الكائن. يفيد هذا
الماكرو عندما يحتاج الكائن للتهيئة بمعطيات، إذ يمكن حينها استدعاء `~هيئ` يدويًا بعد الحجز.

```
عرف ك: سند[صـنفي](احجز_كائنا[صـنفي])؛
ك~هيئ(معطى)؛
```
...
ماكرو يحجز ذاكرة لكائن من الصنف المعطى ويهيئه دون معطيات، ويعيد سندًا لذلك الكائن. يستلم معطىً واحدًا هو صنف الكائن.
يجب تحرير الكائنات المحجوزة بهذا الماكرو لاحقًا باستخدام `حرر_كائنا`.

```
عرف ك: سند[صـنفي](أنشئ_كائنا[صـنفي])؛
```
...
ماكرو يُتلف كائنًا ويحرر الذاكرة المحجوزة له. له صيغتان:
...
الصيغة الأولى تستلم سند الكائن المراد تحريره. أما الصيغة الثانية فتستلم إضافة لذلك صنفًا، وتُمثل الكائن بذلك الصنف (باستخدام
`مثل_سند`) قبل إتلافه؛ يفيد هذا عندما تملك سندا من نوع جذر على كائن من نوع فرع، وتحتاج لتمثيله للصنف الفرع قبل استدعاء
عملية الإتلاف لإتلاف الكائن وتحريره بشكل صحيح.

#### حرر_مؤشر_كائن (freeObjPtr)

ماكرو مشابه لـ `حرر_كائنا` لكنه يستلم مؤشرًا اعتياديًا بدلًا من سند. يستلم معطيين: مؤشر الكائن، وصنف الكائن الذي يُستخدم
لتمثيل المؤشر قبل إتلاف الكائن وتحرير ذاكرته.

```
حرر_مؤشر_كائن[م، صـنفي]؛
```

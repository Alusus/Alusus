...
def _process_args():
    ...
    parser.add_argument("--numthreads", metavar="NUM_THREADS", help="Specify a custom number of threads to use when building dependencies " +
                                                                    "and Alusus (default is \"{}\" on the current system).".format(
    ...
    if args.bloc:
        ...
        global_args['buildsPath'] = os.path.join(
            global_args['productPath'], "Builds")
        global_args['buildPath'] = os.path.join(
            global_args['buildsPath'], global_args['buildType'][0].upper() + global_args['buildType'][1:])
        global_args['depsPath'] = os.path.join(
            global_args['buildsPath'], "Dependencies")
        global_args['pythonDepsPath'] = os.path.join(
            global_args['depsPath'], "PythonDeps")
        if not os.path.exists(global_args['pythonDepsPath']):
            os.makedirs(global_args['pythonDepsPath'])
        global_args['localSitePackagesPath'] = get_local_site_packages(
            global_args['pythonDepsPath'])
        global_args['packagesPath'] = os.path.join(
            global_args['buildsPath'], "Packages")
        global_args['installPath'] = os.path.join(
            global_args['productPath'], "LocalInstall", global_args['buildType'][0].upper(
            ) + global_args['buildType'][1:]
        )

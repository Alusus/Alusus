...
if [[ ! -e ${LLVM_NAME}.install ]]; then
  ...
  cd ${LLVM_NAME}.build
  cmake ../${LLVM_NAME}.src -DCMAKE_BUILD_TYPE="release" -DCMAKE_INSTALL_PREFIX=$DEPS_PATH/${LLVM_NAME}.install
...
cmake $ALUSUS_ROOT/Sources -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DLLVM_PATH=$DEPS_PATH/${LLVM_NAME}.install -DCMAKE_INSTALL_PREFIX=${INSTALL_PATH}

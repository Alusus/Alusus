...
def restore_git_tree_to_path(repo_path, git_hash: str, path: str, verbose_output=False) -> None:
    os.makedirs(path, exist_ok=True)
    ret = subprocess_run_hidden_except_on_error(
        generate_git_cmd(["restore", "-q", "-s", git_hash, "."], repo_path),
        verbose_output=verbose_output, cwd=path)
    if ret.returncode:
        raise OSError(ret)


def get_current_branch(repo_path, verbose_output=False) -> str:
    ret = subprocess_run_hidden_except_on_error(
...
def describe_tags(repo_path, verbose_output=False) -> str:
    ret = subprocess_run_hidden_except_on_error(
